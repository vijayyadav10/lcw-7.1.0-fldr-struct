...
export default class ContentDetailModal extends Component {
    ...
    toggleTab = (index) => {
        ...
        if (this.state.dataToShowOnModal && this.state.dataToShowOnModal.localizations && this.state.dataToShowOnModal.localizations.length > 0) {
            ...
            <Modal dialogClassName="ContentsFilterModal" show={this.props.show} onHide={this.props.onHide} >
                {this.state.loadingData ?
                    ...
                    <>
                        ...
                        <Modal.Body>
                            <div className="CollapsibleSection__title no-padding" role="button" tabIndex={0}></div>
                            <div>
                                <Tabs id={'id'} activeKey={this.props.activeTabKey} onSelect={this.toggleTab}>
                                    {this.state.dataToShowOnModal && this.state.dataToShowOnModal.locale ?
                                        <Tab eventKey={0} title={this.getLocaleFullName(this.state.dataToShowOnModal.locale).split(" ")[0]} className="cdm-lang-tab">
                                            {renderPanelGroup(this.state.dataToShowOnModal, this.state.strapiBaseUrl)}
                                        </Tab> :
                                        <Tab eventKey={0} title="English" className="cdm-lang-tab">
                                            {renderPanelGroup(this.state.dataToShowOnModal, this.state.strapiBaseUrl)}
                                        </Tab>}
                                    {this.state.dataToShowOnModal && this.state.dataToShowOnModal.localizations && this.state.dataToShowOnModal.localizations.length > 0 && this.state.dataToShowOnModal.localizations.map((locTabs, i) => {
                                        ...
                                        return <Tab key={i + 1} eventKey={i + 1} title={this.getLocaleFullName(locTabs.locale).split(" ")[0]} className="cdm-lang-tab">
